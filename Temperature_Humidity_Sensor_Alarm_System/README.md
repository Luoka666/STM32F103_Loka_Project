...
-   **阈值报警**：温湿度超限后 LED 自动闪烁报警（非阻塞式，可扩展蜂鸣器联动）
...
├── Record_storage.c/.h // 历史记录存储（环形缓冲区）
├── LED.c / .h          // LED 状态控制
└── delay.c / .h        // 微秒/毫秒延时函数
```
...
时序错配：非阻塞版 Key_GetNum() 采用“按键事件触发 + 读取后自动清零”的设计。当它放在高频轮询区时，键值在 100ms 状态机执行前就被清零，导致状态机永远收不到有效按键。

### 解决方案04
延时函数重构：Delay_ms 改为基于 g_millis（由 SysTick 中断维护）的阻塞实现，Delay_us 改为纯软件循环。两者都不再直接操作 SysTick->CTRL 寄存器，从根源上保护系统心跳。
